...
import type { CapsuleDetailRes, CapsuleListsRes, CapsuleSortType } from "@/shared/types/api/capsule";
import { apiClient } from "../api-client";

export const capsuleQueryKeys = {
  all: () => ["capsule"],
  detail: (id: string) => [...capsuleQueryKeys.all(), id],
  lists: (page?: number, size?: number, sort?: CapsuleSortType, type?: string) => [
...
export const capsuleQueryOptions = {
  ...
  capsuleLists: (page?: number, size?: number, sort?: CapsuleSortType, type?: string) =>
    queryOptions({
...
const getCapsuleLists = (page?: number, size?: number, sort?: CapsuleSortType, type?: string) => {
  return apiClient.get<CapsuleListsRes>(ENDPOINTS.CAPSULE_LISTS(page, size, sort, type));
};
